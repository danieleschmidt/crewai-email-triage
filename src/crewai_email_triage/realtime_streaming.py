...
from .llm_pipeline import get_llm_pipeline, LLMResponse
...
class StreamSubscription:
    ...
    def matches_event(self, event: StreamEvent) -> bool:
        ...
        for filter_key, filter_value in self.filters.items():
            if filter_key == "session_id" and event.session_id != filter_value:
                return False
            elif filter_key == "user_id" and event.user_id != filter_value:
                return False
            elif filter_key in event.data and event.data[filter_key] != filter_value:
                return False
